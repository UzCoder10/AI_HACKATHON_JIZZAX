...
import { generateWeeklyReport, generateBasicReport } from "@/lib/insights/reportGenerator";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = reportAccess.allowed
      ? await generateWeeklyReport({ childId, language, periodDays: days })
      : await generateBasicReport({ childId, language, periodDays: days });

    const [moodEntries, alerts] = await Promise.all([
    ...
    return NextResponse.json({
      ...
      data: {
        child,
        report,
        tier: reportAccess.allowed ? "full" : "basic",
